...
```
There are `r ncol(early_january_weather)` variables in this dataset named `early_january_weather`. The variable names are `origin`, `year`, `month`, `day`, `hour`, `temp`, `dewp`, `humid`, `wind_dir`, `wind_speed`, `wind_gust`, `precip`, `pressure`, `visib`, `time_hour`. The dataset has `r nrow(early_january_weather)` rows and `r ncol(early_january_weather)` columns. The mean temperature is `r mean(temps)` degrees.
